"use strict";
const stripe = require("stripe")("sk_test_sFcp1zGZU2rJLjwy9vCA4p3w00fIqoUZFR");
...
module.exports = {
  ...
  create: async ctx => {
    ...
    let subtotal_in_cents = parseInt(
      strapi.config.functions.cart.cartSubtotal(sanitizedCart)
    );
    console.log("subtotal_in_cents", subtotal_in_cents);
    let taxes_in_cents = parseInt(
      strapi.config.functions.cart.cartTaxes(sanitizedCart)
    );
    console.log("taxes_in_cents", taxes_in_cents);
    let total_in_cents = parseInt(
      strapi.config.functions.cart.cartTotal(sanitizedCart)
    );
